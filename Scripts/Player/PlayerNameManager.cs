...
public class PlayerNameManager : NetworkBehaviour
{
    ...
    }    private void OnPlayerNameChanged(FixedString32Bytes oldName, FixedString32Bytes newName)
    {
        ...
        // BUG FIX: Update lobby data when host name changes
        // This solves the issue where host name changes aren't reflected in the room list
        // when the host is alone in the room
        if (IsOwner && IsServer && !newName.IsEmpty)
        {
            Debug.Log($"[PlayerNameManager] Host name changed - updating UGS Lobby data");
            LobbyManager lobbyManager = LobbyManager.Instance;
            if (lobbyManager != null)
            {
                // Force update lobby data with new host name
                _ = lobbyManager.UpdateLobbyPlayerDataAsync();
                
                // Find and notify RoomListManager to refresh after a delay
                // This needs to happen AFTER the lobby data is updated through UGS
                StartCoroutine(NotifyRoomListManagerAfterDelay(2.0f));
            }
        }
    }
    
    private System.Collections.IEnumerator NotifyRoomListManagerAfterDelay(float seconds)
    {
        yield return new WaitForSeconds(seconds);
        
        // Find RoomListManager in the scene
        RoomListManager roomListManager = FindObjectOfType<RoomListManager>();
        if (roomListManager != null)
        {
            Debug.Log("[PlayerNameManager] Notifying RoomListManager to refresh after host name change");
            roomListManager.RefreshAfterHostNameChange();
        }
    }
