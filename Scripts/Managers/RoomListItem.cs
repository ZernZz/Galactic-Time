...
public class RoomListItem : MonoBehaviour
{
    ...
    public void Setup(RoomInfo roomInfo, RoomListManager manager)
    {
        ...
        if (joinButton != null)
        {
            ...
            // --- IMPORTANT: Disable Join button if Join Code is not available or "N/A" ---
            // This relies on the Join Code being retrieved correctly (check RoomListManager notes on visibility)
            bool canJoin = !(string.IsNullOrEmpty(roomInfo.joinCode) || roomInfo.joinCode == "N/A");
            ...
            if (buttonText != null) buttonText.text = canJoin ? "Join" : "Info"; // Or "Unavailable" / "View"
            // ------------------------------------------------------------------------------
        }
        else
        {
