...
public class LobbyUI : MonoBehaviour
{
    ...
    private async void OnHostClicked()
    {
        ...
        try
        {
            ...
            Debug.Log($"[LobbyUI] StartHostAsync likely succeeded ({ConnectionManager.Instance?.JoinCode}). Attempting to load Lobby Scene...");
            LoadLobbyScene();
            // --- END MODIFIED PART ---
        }
    ...
    private void OnQuickMatchClicked()
    {
        if (roomListManager != null)
        {
            ...
            // รีเซ็ตปุ่ม QuickMatch หลังจาก 1 วินาที โดยไม่สนใจผลลัพธ์
            // รีเซ็ตปุ่มทั้งหมดหลัง 1 วินาที
            StartCoroutine(ResetQuickMatchButtonAfterDelay(1.0f));
        }
        else
        {
      ...
      // เพิ่มเมธอดใหม่สำหรับรีเซ็ตปุ่มทั้งหมดในแผง Room Panel หลังจากผ่านไป 1 วินาที
    private IEnumerator ResetQuickMatchButtonAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
        // รีเซ็ตปุ่มทั้งหมดในแผง Room Panel
        if (roomPanel_QuickMatchButton != null)
        {
            roomPanel_QuickMatchButton.interactable = true;
        }
        if (roomPanel_HostButton != null)
        {
            roomPanel_HostButton.interactable = true;
        }
        if (roomPanel_JoinByCodeButton != null)
        {
            roomPanel_JoinByCodeButton.interactable = true;
        }
        if (roomPanel_CancelButton != null)
        {
            roomPanel_CancelButton.interactable = true;
        }
    }
    ...
    private void CheckAndUpdateJoinCode()
    {
         ...
         if (isCodeReady) { joinCodeDisplayText.text = $"{currentJoinCode}"; needsJoinCodeCheck = false; if(copyCodeButton != null && NetworkManager.Singleton.IsHost) copyCodeButton.interactable = true; }
         else { joinCodeDisplayText.text = "Generating..."; if(NetworkManager.Singleton.IsHost) needsJoinCodeCheck = true; if(copyCodeButton != null) copyCodeButton.interactable = false; }
    }
    ...
    public void UpdatePrivacyButton(bool isPublic) { if (currentSceneType == SceneType.Lobby && privacyToggleButton != null) { privacyToggleButton.interactable = NetworkManager.Singleton.IsHost; if (privacyButtonText != null) { privacyButtonText.text = isPublic ? "Private" : "Public"; } } }
