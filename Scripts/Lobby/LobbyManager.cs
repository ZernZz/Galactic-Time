...
public class LobbyManager : NetworkBehaviour
{
    ...
    private async Task CreateLobbyAsync(string relayJoinCode, bool startPublic)
    {
        ...
        try
        {
            ...
            string lobbyName = $"{hostName}'s Game"; // Example Lobby name
    ...
    private async Task UpdateLobbyPlayerDataAsync()
    {
        if (!IsServer || currentLobby == null || isShuttingDown) return;
        ...
        try
        {
             ...
             // Check Host Name
              if (!currentLobby.Data.TryGetValue(KEY_HOST_NAME, out var currentHostNameData) || currentHostNameData.Value != hostName)
              {
                  dataToUpdate[KEY_HOST_NAME] = new DataObject(DataObject.VisibilityOptions.Public, hostName);
                  changed = true;
              }

              // Add other data fields to check/update if needed...
              // if (!currentLobby.Data.TryGetValue("GameMode", out var gmData) || gmData.Value != "NewMode") {
              //    dataToUpdate["GameMode"] = new DataObject(DataObject.VisibilityOptions.Public, "NewMode");
              //    changed = true;
              // }
             ...
             if (changed)
             {
                  ...
                  // Debug.Log($"[LobbyManager] Updating UGS Lobby ({currentLobby.Id}) Data...");
                  currentLobby = await LobbyService.Instance.UpdateLobbyAsync(currentLobby.Id, options);
                  // Debug.Log($"[LobbyManager] UGS Lobby data updated.");
             } else {
